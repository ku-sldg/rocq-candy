...
Theorem map_get_works{A B:Type} `{H : EqClass A} : forall m (x:A) (v:B),
...
Lemma map_get_distinct_keys{A B:Type} `{H : EqClass A} : 
...
Lemma map_get_distinct_keys_from_set {A B :Type} `{H : EqClass A} : forall (m : Map A B) k1 k2 v1 v2,
  k1 <> k2 ->
  map_get k2 (map_set k1 v1 m) = Some v2 ->
  map_get k2 m = Some v2.
Proof.
